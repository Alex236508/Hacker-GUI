...
addBtn(vfx,'Explode Page',()=>{if(window.explodeInt) clearInterval(window.explodeInt); let o=...; window.explodeInt=setInterval(...);},()=>{if(window.explodeInt) clearInterval(window.explodeInt);});
...
addBtn(vfx,'Disco Mode',()=>{window.discoInt=setInterval(()=>{document.querySelectorAll('*:not(#vfxGUI *):not(#utilitiesGUI *)').forEach(e=>{e.style.backgroundColor=['red','orange','yellow','green','blue','purple','pink'][Math.floor(Math.random()*7)]});},100);},()=>{clearInterval(window.discoInt); window.discoInt=null; document.querySelectorAll('*:not(#vfxGUI *):not(#utilitiesGUI *)').forEach(e=>e.style.backgroundColor='');});
...
let originalTexts = [];
addBtn(vfx,'Bubble Text',()=>{originalTexts=[];window.bubbleInt=setInterval(()=>{function transform(el){if(el.id==='vfxGUI'||el.id==='utilitiesGUI'||el.closest('#vfxGUI,#utilitiesGUI'))return;if(el.childNodes.length>0)el.childNodes.forEach(transform); if(el.nodeType===Node.TEXT_NODE&&el.nodeValue){originalTexts.push({el,text:el.nodeValue}); let chars='abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'.split(''); let bubbles='ⓐⓑⓒⓓⓔ...'; el.textContent=el.textContent.replace(/[a-zA-Z0-9]/g,l=>bubbles[chars.indexOf(l)]);}} transform(document.body);},50);},()=>{clearInterval(window.bubbleInt); window.bubbleInt=null; originalTexts.forEach(o=>o.el.textContent=o.text);});
