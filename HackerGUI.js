(function(){
  ...
  function spawnGUIs() {
    ...
    function addBtn(container, name, on, off=()=>{}) {
      ...
      btn.innerText = name;
      btn.style.cssText = 'margin:3px 0;padding:4px 6px;background:#111;border:1px solid #00ff00;border-radius:4px;cursor:pointer;text-align:center;';
      let active = false;
      btn.onclick = () => { active=!active; if(active) on(); else off(); };
