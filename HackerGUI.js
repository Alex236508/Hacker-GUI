...
addBtn(vfx,'Disco Mode',()=>{if(window.discoActive)return;window.discoActive=true;window.discoInt=setInterval(()=>{document.querySelectorAll('body *:not(#vfxGUI *):not(#utilitiesGUI):not(#utilitiesGUI *)').forEach(e=>{e.style.backgroundColor=['red','orange','yellow','green','blue','purple','pink'][Math.floor(Math.random()*7)]});},100);},()=>{clearInterval(window.discoInt);window.discoInt=null;window.discoActive=false;document.querySelectorAll('body *:not(#vfxGUI *):not(#utilitiesGUI):not(#utilitiesGUI *)').forEach(e=>e.style.backgroundColor='');vfx.style.backgroundColor='#1b1b1b';});
