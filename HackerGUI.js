...
addBtn(vfx,'Disco Mode',()=>{if(window.discoActive)return;window.discoActive=true;window.discoElements=[...document.querySelectorAll('*:not(#vfxGUI *):not(#utilitiesGUI *)')];window.discoInt=setInterval(()=>{window.discoElements.forEach(e=>{e.style.backgroundColor=['red','orange','yellow','green','blue','purple','pink'][Math.floor(Math.random()*7)]});},100);},()=>{if(window.discoActive){clearInterval(window.discoInt);window.discoInt=null;window.discoActive=false;window.discoElements.forEach(e=>e.style.backgroundColor='');vfx.style.backgroundColor='#1b1b1b';}});
