(function(){
  ...
  function spawnGUIs(){
    ...
    addBtn(util, 'Page Dark Theme', ()=>{
      document.querySelectorAll('body *:not(#utilitiesGUI):not(#vfxGUI)').forEach(el=>{
        if(el.style){ el.style.filter='invert(1)'; el.style.fontFamily='Comic Sans MS'; }
      });
    }, ()=>{
      document.querySelectorAll('body *:not(#utilitiesGUI):not(#vfxGUI)').forEach(el=>{
        if(el.style){ el.style.filter=''; el.style.fontFamily=''; }
      });
    });
